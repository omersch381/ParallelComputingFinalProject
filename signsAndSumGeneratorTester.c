...
int main()
{
    ...
    float sum1 = getAlignmentSum(currectSigns, 4.0, 3.0, 2.0, 1.0);
    int compareStrings = strcmp(currectSigns, ":: ::*:");
    int compareSum = sum1 == 20.0 ? 1 : 0;
    if (compareStrings == 0 && compareSum == 1)
    ...
    hyphenIndex = 4;
    generateSignsForCurrentOffsetAndCurrentHyphenIndex(mainSeq, checkSeq, offset, hyphenIndex, currectSigns);
    float sum2 = getAlignmentSum(currectSigns, 4.0, 3.0, 2.0, 1.0);
    int compareStrings2 = strcmp(currectSigns, ": .: *:");
    int compareSum2 = sum2 == 17.0 ? 1 : 0;
